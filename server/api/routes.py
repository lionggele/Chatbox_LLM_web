...
def query_llms():
    ...
    responses = {}
    if selected_model in engines:
        engine = engines[selected_model]
        responses[selected_model] = engine.send(query).text
    else:
        responses[selected_model] = f"Error: Model '{selected_model}' not found."
...
def get_model_averages():
    try:
        ...
        results = EvaluationResult.query.filter(EvaluationResult.experiment_id.contains(dataset)).all()
